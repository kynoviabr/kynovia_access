...
function residentsPath(condominiumId: string) {
  return `/dashboard/condominiums/${condominiumId}/residents`;
}

function redirectToResidents(condominiumId: string, status: string) {
  revalidatePath(residentsPath(condominiumId));
  redirect(`${residentsPath(condominiumId)}?status=${status}`);
}
...
export async function createResidentAction(formData: FormData) {
  ...
  if (!condominiumId || !fullName || !isResidentStatus(status)) {
    redirectToResidents(condominiumId, "missing_resident_fields");
  }
  ...
  if (error) {
    redirectToResidents(condominiumId, "create_resident_failed");
  }

  redirectToResidents(condominiumId, "resident_created");
}
...
export async function updateResidentAction(formData: FormData) {
  ...
  if (error) {
    redirectToResidents(condominiumId, "update_resident_failed");
  }

  redirectToResidents(condominiumId, "resident_updated");
}
...
export async function deleteResidentAction(formData: FormData) {
  ...
  if (!condominiumId || !residentId) {
    redirectToResidents(condominiumId, "missing_resident_id");
  }
  ...
  if (error) {
    redirectToResidents(condominiumId, "delete_resident_failed");
  }

  redirectToResidents(condominiumId, "resident_deleted");
}
...
export async function linkResidentUnitAction(formData: FormData) {
  ...
  if (!condominiumId || !residentId || !unitId || !isResidentUnitRelationship(relationship)) {
    redirectToResidents(condominiumId, "missing_unit_link_fields");
  }
  ...
  if (error) {
    redirectToResidents(condominiumId, "link_unit_failed");
  }

  redirectToResidents(condominiumId, "unit_linked");
}
...
export async function unlinkResidentUnitAction(formData: FormData) {
  ...
  if (!condominiumId || !residentUnitId) {
    redirectToResidents(condominiumId, "missing_unit_link_id");
  }
  ...
  if (error) {
    redirectToResidents(condominiumId, "unlink_unit_failed");
  }

  redirectToResidents(condominiumId, "unit_unlinked");
}
...
export async function createResidentVehicleAction(formData: FormData) {
  ...
  if (!condominiumId || !residentId || !isLikelyBrazilianPlate(plate)) {
    redirectToResidents(condominiumId, "invalid_vehicle_plate");
  }
  ...
  if (error) {
    redirectToResidents(condominiumId, "create_vehicle_failed");
  }

  redirectToResidents(condominiumId, "vehicle_created");
}
...
export async function updateResidentVehicleAction(formData: FormData) {
  ...
  if (!condominiumId || !vehicleId || !isLikelyBrazilianPlate(plate) || !isResidentStatus(status)) {
    redirectToResidents(condominiumId, "invalid_vehicle_fields");
  }
  ...
  if (error) {
    redirectToResidents(condominiumId, "update_vehicle_failed");
  }

  redirectToResidents(condominiumId, "vehicle_updated");
}
...
export async function deleteResidentVehicleAction(formData: FormData) {
  ...
  if (!condominiumId || !vehicleId) {
    redirectToResidents(condominiumId, "missing_vehicle_id");
  }
  ...
  if (error) {
    redirectToResidents(condominiumId, "delete_vehicle_failed");
  }

  redirectToResidents(condominiumId, "vehicle_deleted");
}
